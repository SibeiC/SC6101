...
In this paper, we use \emph{privacy leakage} as an umbrella term that covers (i) \emph{linkability} (linking addresses, users, and activities) and (ii) \emph{confidentiality loss} (exposure of amounts, positions, or relationships). We describe adversaries using a common threat-model vocabulary: a global on-chain observer (GPA), a network/RPC observer (NLA), and a privileged compliance actor (e.g., issuer/VASP). We also group impacts into four recurring dimensions: \emph{identity exposure}, \emph{financial exposure} (balances or positions), \emph{strategic exploitation} (e.g., MEV, liquidation sniping), and \emph{compliance/control risk} (freezing, blacklisting, censorship).
